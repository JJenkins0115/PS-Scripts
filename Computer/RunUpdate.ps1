   # Computer/RunUpdate.ps1
   Write-Host "Checking for Windows Updates (Zero-Prompt Mode)..."
    $ModuleName = "PSWindowsUpdate"
    
    try {
        # 1. Force NuGet Provider installation (The "Do you want to install NuGet?" prompt)
        Write-host "Configuring NuGet Provider..."
        ...
        Write-Host "Downloading and installing updates (No Prompts)..."
        ...
        Write-Host "Windows Updates processed."
    } catch { 
        Write-Host "Update process skipped: $($_.Exception.Message)"
    }
